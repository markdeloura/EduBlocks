...
export default {
	...
	data() {
		return {
			...
			filledIconsWithTriplePaths: ["database", "finger_print"]
		};
	},
	...
	computed: {
		...
		strokeStyleComponent() {
			if (this.isFilled) {
				return this.strokeWidth !== 2 ? this.strokeWidth : 0;
			}
			else {
		...
		fillStyleComponent() {
			if (this.isFilled) {
				return this.fillColor !== "none" ? this.fillColor : "currentColor";
			}
			else {
		...
		fillRuleBasicPath() {
			if (this.isFilled) {
				return this.fillRule !== "nonzero" ? this.fillRule : "evenodd";
			}
			else {
		...
		clipRuleBasicPath() {
			if (this.isFilled) {
				return this.clipRuleData !== "nonzero" ? this.clipRuleData : "evenodd";
			}
			else {
	...
	methods: {
		buildIcon() {
			if (
				!this.iconsWithDoublePath.includes(`${this.name}`) &&
        !this.isFilled &&
        !this.filledIconsWithTriplePaths.includes(`${this.name}`)
			) {
		...
		buildIconDoublePath() {
			...
			const temporalJSONWithDoblePath = this.icosWithDoublePath();
		...
		buildIconDoublePathFilled() {
			...
			const temporalJSONWithDoblePath = this.icosWithDoublePathFilled();
		...
		buildIconTriplePathFilled() {
			...
			const temporalJSONWithDoblePath = this.icosWithTriplePathFilled();
